...
const questionableContentButton = document.querySelector(
...
function playPlaylist(tag, videoId) {
  ...
  playlist = shuffle(Array.from(Array(count).keys()).map((n) => ++n));

  videoIndexes[NEXT] = 0;

  const videoDataPromise = videoId
    ? fetchVideoDataById(videoId)
    : fetchVideoData(videoIndexes[NEXT], 1);
  videoDataPromise
    .then((data) => {
...
function previousFrame() {
  videoPlayers[CURRENT].currentTime = Math.max(
    0,
    videoPlayers[CURRENT].currentTime - FRAME_RATE
  );
}

function nextFrame() {
  videoPlayers[CURRENT].currentTime = Math.min(
    videoPlayers[CURRENT].duration,
    videoPlayers[CURRENT].currentTime + FRAME_RATE
  );
}
function isPaused() {
  return videoPlayers[CURRENT].paused || videoPlayers[CURRENT].ended;
}
...
function playPreloadedVideo(direction) {
  ...
  videoIndexes[nextVideoIndexToPreload] = shiftIndex(
    videoIndexes[CURRENT],
    direction
  );
  ...
  playVideo();
  fetchVideoData(videoIndexes[nextVideoIndexToPreload], direction).then(
    (data) => {
...
function showTitleAfterDelay(tags) {
  ...
  if (showTitlesButton.checked) {
    const longestPossibleDelay =
      videoPlayers[CURRENT].duration * 1000 - SHOW_TITLE_DURATION;
    const delayUntilTitle = Math.min(
      options["title-delay"] * 1000,
      longestPossibleDelay
    );
  ...
  if (tag !== parseTagFromUrl()) {
    if (tag) {
      history.pushState(
        { tag: tag },
        tag + " videos",
        "?tag=" + encodeURIComponent(useUnderscores(tag))
      );
    } else {
...
function putTagsInUi(tags) {
  tagsByName = new Map(
    tags.filter(shouldIncludeTag).map((tag) => [makeReadable(tag.name), tag])
  );
